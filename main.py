...
file_path = 'C:/Users/Eugene/Downloads/anonymous-msweb.data'
try:
...
for line in f:
    ...
    if s[0] == 'C':
        usr = int(s[2])
        if usr_web != []:
            data_usr.append(usr_web)
        usr_web = []
    elif s[0] == 'V':
        if int(s[1]) not in web_cnt.keys():
            web_cnt[int(s[1])] = 1
        else:
            web_cnt[int(s[1])] += 1
        data_pair.append((usr, int(s[1])))
        usr_web.append(int(s[1]))
    elif s[0] == 'A':
        atr = ''
...
atr = []
for i in df[0]:
    atr.append(web_atr[i])
print(atr)
df = pd.DataFrame(letter_counts.most_common(30), index=atr)
df = df[1]
...
d_data = d.fit(data_usr).transform(data_usr)
df = pd.DataFrame(d_data, columns=d.columns_)
df1 = apriori(df, min_support=0.002, use_colnames=True)
df1.sort_values(by='support', ascending=False)
association_rule = association_rules(df1, metric='confidence')
association_rule = association_rule.sort_values(by=['confidence', 'lift'], ascending=False)
print(association_rule)
out_path = 'data_result.csv'
association_rule.to_csv(out_path, index=True)

minSup = 2
initSet = createInitSet(data_usr)
